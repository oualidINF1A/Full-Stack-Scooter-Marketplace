...
import { Advert, OfferType } from '../types'
...
export const AdvertPage = () => {
    ...
    const getOffers = async () => {
        ...
        if(!res.data.succes) return
        const sortedOffers = res.data.offers.sort((a:OfferType, b:OfferType) => {
...
${user.name}.
    ...
    <div> 
        ...
        <div className='px-16 py-4 lg:flex hidden lg:flex-row flex-col justify-center gap-2'>
            ...
            <div className=' p-4 bg-white rounded-sm border w-3/5 shadow-sm flex flex-col gap-4 xl:min-w-fit'>
                <div className='flex xl:flex-row flex-col justify-center items-center'>
                    <div>
                        ...
                        <div className='max-w-lg rounded-xl h-[500px] shadow-md'
                        >
                    ...
                    <div className='mt-16 mx-6 flex flex-col gap-8'>
                        ...
                        <div className='flex flex-col gap-2 bg-gray-100 shadow-sm p-4 rounded-2xl'>
                            ...
                            {advert.scooterPart && (
                                <h1 className='text-gray-600 flex gap-2'>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className='w-6 h-6'><path d="M470.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L192 338.7 425.4 105.4c12.5-12.5 32.8-12.5 45.3 0z"/></svg>
                                 Conditie: <span className='italic'>{advert.scooterPart.condition}</span></h1>
                            )}
                ...
                <div className='border-t-4 py-2'>
                        ...
                        {/**Bieden gedeelte */}
            <div className='px-2 py-4 bg-white lg:max-w-[515px] lg:min-w-[300px]  flex flex-col gap-1 h-fit'>
        ...
        <div className='lg:hidden flex flex-col gap-2'>
            ...
            {/*Fotos */}
            <div className='w-full flex justify-center items-center rounded-xl shadow-md'
                    >
                        <Carousel advertId={advert._id}>
                            {advert.images.map((image, i) => (
                                    <img src={image} alt="" key={i} className=' min-w-[100vw] max-h-[300px] W-[400px] object-contain'/>
                                ))}
                        </Carousel>
                ...
                <div className='flex flex-col justify-between items-center py-4 bg-white px-1'>
                ...
                <div className='flex flex-col gap-2 bg-white shadow-sm p-4 rounded-2xl'>
                                    <h1 className='text-xl font-bold '>Beschrijving:</h1>
                                    <p className='text-gray-600'>{advert.description}</p>
                </div>

                {/*Contact */}
                <div className='flex flex-col gap-4 h-fit bg-white py-4 px-8 border'>
                                <Link to={`/account/${advert.owner._id}`}
                                className='text-indigo-500 hover:underline'
                                >{advert.owner.name}</Link>
                                {advert.showContactInfo && (
                ...
                {/**Bieden gedeelte */}
                <div className='px-2 py-4 mb-8 bg-white w-full flex flex-col gap-1 h-fit'>
                            {notLoggedInError && <ErrorMessage text='U moet ingelogd zijn om te kunnen bieden'/>}
                            {offerTooLowError && <ErrorMessage text='Bod moet hoger zijn dan minimaal bod'/>}
                            {offerLowerThanLast && <ErrorMessage text='Bod moet hoger zijn dan laatste bod.'/>}
                            {alreadyHighestBidderError && <ErrorMessage text='U bent al de hoogste bieder.'/>}
                            ...
                            {user._id === advert?.owner._id && advert.offers.length == 0 && (
                                <span className='text-gray-600 py-4 text-center font-bold'>Er zijn nog geen biedingen</span>
                            )}
